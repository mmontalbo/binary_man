...
Goal: Skip wasteful bare auto-verify cycle by sending ALL surface items with
descriptions to LM in the first behavior cycle, letting it generate complete
scenarios with appropriate stdin/fixtures immediately.

### Motivation

The old flow wasted cycles:
```
Cycle 1: Surface discovery (help scenario)
Cycle 2: Auto-verify bare options → Run → outputs_equal (99% fail)
Cycle 3: LM prompted with failures → Creates scenarios with stdin/seed
Cycle 4: Run → Some verify, some still fail
Cycle 5+: Fix remaining
```

Bare auto-verify scenarios almost always produce `outputs_equal` because:
- Text filters (cut, sort, tr) need stdin
- File operations (touch, mkdir) need file assertions
- Modifiers (--complement) need companion options

### Solution: Initial Scenarios Reason Code

New `initial_scenarios` reason triggers when surface exists but no behavior
scenarios in inventory. LM receives ALL surface items with descriptions upfront.

```
Cycle 1: Surface discovery (help scenario - unchanged)
Cycle 2: LM generates scenarios for ALL options (with descriptions)
Cycle 3: Run scenarios → Many verify, failures have real delta evidence
Cycle 4: LM fixes remaining failures (existing flow)
```
...
| File | Change |
|------|--------|
| `src/enrich/types.rs` | Add `SurfaceItemHint` type and `all_surface_items` to `ScaffoldContext` |
| `src/status/verification_policy.rs` | Add `InitialScenarios` variant to `BehaviorReasonKind` |
| `src/status/evaluate/verification_requirement/next_action.rs` | Detect initial behavior cycle |
| `src/status/evaluate/verification_requirement/scaffold.rs` | Populate `all_surface_items` |
| `src/status/evaluate/verification_requirement/mod.rs` | Clear auto-verify next_action in initial state |
| `src/workflow/apply/auto_verify.rs` | Skip bare scenarios when no behavior scenarios exist |
| `src/workflow/lm_client.rs` | Handle `initial_scenarios` reason with all surface items |
| `prompts/behavior_reason_initial_scenarios.md` | New prompt template |

### E2E Results

| Binary | Options | Verified | Cycles | Notes |
|--------|---------|----------|--------|-------|
| **ls** | 84 | 100% | 6 | Complex, many fixture types |
| **tr** | 11 | 100% | 4 | Text filter, stdin-based |
| **cut** | 17 | 100% | 3 | Text filter, stdin-based |
| **wc** | 14 | 100% | 3 | Text filter |
| **date** | 19 | 95% | 3 | 1 fixture gap |
| **git config** | 32 | 100% | 5 | Subcommand context |
| **sort** | 52 | 23% | 5 | Needs exit_code assertions |
| **cp** | 51 | 10% | 4 | File assertions struggling |

### Acceptance Criteria

| Criterion | Status |
|-----------|--------|
| Skip bare auto-verify for initial cycle | ✓ |
| LM receives option descriptions | ✓ |
| `ls` reaches 100% verification | ✓ |
| `tr` reaches 100% verification | ✓ |
| No regression in existing tests | ✓ |
| Default `max_cycles` reduced to 15 | ✓ |

### Remaining Gaps (deferred)

1. **Exit code assertions** - `sort --check` needs non-zero exit verification
2. **File operation assertions** - `cp`, `mv` scenarios need better generation
3. **Blocking command detection** - `cat`, `tail -f` need automatic exclusion
