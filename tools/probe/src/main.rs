...
fn cmd_run(binary: &str, test_path: &PathBuf) -> Result<()> {
    ...
    for run in &script.runs {
        for ctx in &script.contexts {
            if let Some(ref scoped) = run.in_contexts {
                if !scoped.contains(&ctx.name) { continue; }
            }
            actual_runs += 1;
        }
    }
    ...
    for (ri, run) in script.runs.iter().enumerate() {
        ...
        if !sensitive.is_empty() {
            out.push_str(&format!("  sensitive to: {}\n",
                sensitive.iter()
        ...
        let all_no_fs = obs_list.iter().all(|(_, o)| o.fs_changes.is_empty());
        ...
        if all_no_fs && !all_stdout_empty { /* not interesting to note */ }
