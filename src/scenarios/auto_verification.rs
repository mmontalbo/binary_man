...
mod tests {
    ...
    fn auto_verification_inherits_defaults_seed_when_no_prereqs() {
        ...
        assert!(
            scenario.seed.is_none(),
            "auto_verify scenario should have seed: None when no prereqs"
        );
        ...
        assert_ne!(
            digest_a, digest_b,
            "digest should change when defaults seed changes"
        );
    }
    ...
    fn auto_verification_uses_prereqs_seed() {
        use crate::enrich::{PrereqInferenceDefinition, PrereqsFile};
        ...
        prereqs
            .surface_map
            .insert("--color".to_string(), vec!["needs_setup".to_string()]);
        ...
        assert!(
            scenario.seed.is_some(),
            "auto_verify scenario should have seed from prereqs"
        );
